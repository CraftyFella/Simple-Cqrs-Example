...
namespace Inventory.Reporting
{
	public class SQLiteReportingRepository : IReportingRepository
	{
		...
		{
			foreach (var property in dtoType.GetPropertiesAndFields().Where(WhereGeneric))
			{
				foreach (var dto in dtos)
				{
					...
					property.SetValue(dto, childDtos, null);
				}
			}
		}
		...
		private static TDto BuildDto<TDto>(Type dtoType, ConstructorInfo dtoConstructor, IDataRecord sqLiteDataReader) where TDto : class
		{
			...
			dtoType.GetPropertiesAndFields().Where(Where).ToList().ForEach(x => constructorArguments.Add(sqLiteDataReader[x.Name]));
		...
		private static Dictionary<string, object> GetPropertyInformation(object example)
		{
			...
			example.GetType().GetPropertiesAndFields().Where(Where).ToList().ForEach(x => exampleData.Add(x.Name, x.GetValue(example, null)));
			return exampleData;
		}
		...
		private static bool WhereGeneric(IProperty propertyInfo)
		{
			return propertyInfo.PropertyType.IsGenericType;
		}
	}
